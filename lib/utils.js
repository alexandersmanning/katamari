...
const Util = {
	...
	directionOfMove: (objectOne, objectTwo) => {
		let relativeVelocity = Util.vectorSubtraction(objectOne.vel, objectTwo.vel)
		let relativePosition = Util.vectorSubtraction(objectOne.pos, objectTwo.pos)

		return Util.vectorMultiplication(relativeVelocity, relativePosition)
	},

	vectorSubtraction: (vectorOne, vectorTwo) => {
		return [vectorTwo[0] - vectorOne[0], vectorTwo[1] - vectorOne[1]]
	},

	vectorMultiplication: (vectorOne, vectorTwo) => {
